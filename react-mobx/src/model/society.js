import { asMap } from 'mobx';

export default  {
  '0-9': {
    size: 61317,
    voters: 0, 
    weights: asMap({
      'Oświata i wychowanie': 10,
      'Transport i łączność': 0,
      'Gospodarka komunalna i ochrona środowiska': 0,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 5,
      'Gospodarka mieszkaniowa': 0,
      'Pomoc Spoleczna': 4,
      'Administracja publiczna': 0,
      'Edukacyjna opieka wychowawcza': 10,
      'Obsługa długu publicznego': 0,
      'Pozostałe zadania w zakresie polityki społecznej': 4,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 0,
      'Pozostale wydatki': 0
    })
  },
  '10-19': {
    size: 45222,
    voters: 0.02,
    weights: asMap({
      'Oświata i wychowanie': 10,
      'Transport i łączność': 6,
      'Gospodarka komunalna i ochrona środowiska': 0,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 6,
      'Gospodarka mieszkaniowa': 0,
      'Pomoc Spoleczna': 0,
      'Administracja publiczna': 0,
      'Edukacyjna opieka wychowawcza': 0,
      'Obsługa długu publicznego': 0,
      'Pozostałe zadania w zakresie polityki społecznej': 0,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 4,
      'Pozostale wydatki': 0,
    })
  },
  '20-29': {
    size: 88582,
    voters: 0.3,
    weights: asMap({
      'Oświata i wychowanie': 7,
      'Transport i łączność': 8,
      'Gospodarka komunalna i ochrona środowiska': 5,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 10,
      'Gospodarka mieszkaniowa': 7,
      'Pomoc Spoleczna': 3,
      'Administracja publiczna': -4,
      'Edukacyjna opieka wychowawcza': 4,
      'Obsługa długu publicznego': -3,
      'Pozostałe zadania w zakresie polityki społecznej': 2,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 6,
      'Pozostale wydatki': 0,
    })
  },
  '30-39': {
    size: 120268,
    voters: 0.3,
    weights: asMap({
      'Oświata i wychowanie': 3,
      'Transport i łączność': 10,
      'Gospodarka komunalna i ochrona środowiska': 4,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 9,
      'Gospodarka mieszkaniowa': 10,
      'Pomoc Spoleczna': 8,
      'Administracja publiczna': -6,
      'Edukacyjna opieka wychowawcza': 4,
      'Obsługa długu publicznego': -7,
      'Pozostałe zadania w zakresie polityki społecznej': 5,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 8,
      'Pozostale wydatki': 0,
    })
  },
  '40-49': {
    size: 74511,
    voters: 0.3,
    weights: asMap({
      'Oświata i wychowanie': 10,
      'Transport i łączność': 8,
      'Gospodarka komunalna i ochrona środowiska': 10,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 7,
      'Gospodarka mieszkaniowa': 10,
      'Pomoc Spoleczna': 7,
      'Administracja publiczna': -7,
      'Edukacyjna opieka wychowawcza': 7,
      'Obsługa długu publicznego': -8,
      'Pozostałe zadania w zakresie polityki społecznej': 7,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 9,
      'Pozostale wydatki': 0
    })
  },
  '50-59': {
    size: 82526,
    voters: 0.3,
      weights: asMap({
      'Oświata i wychowanie': 3,
      'Transport i łączność': 5,
      'Gospodarka komunalna i ochrona środowiska': 4,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 4,
      'Gospodarka mieszkaniowa': 8,
      'Pomoc Spoleczna': 8,
      'Administracja publiczna': -8,
      'Edukacyjna opieka wychowawcza': 5,
      'Obsługa długu publicznego': -7,
      'Pozostałe zadania w zakresie polityki społecznej': 6,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 4,
      'Pozostale wydatki': 0
    })
  },
  '60-69': {
    size: 86230,
    voters: 0.3,
      weights: asMap({
      'Oświata i wychowanie': 1,
      'Transport i łączność': 10,
      'Gospodarka komunalna i ochrona środowiska': 3,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 4,
      'Gospodarka mieszkaniowa': 10,
      'Pomoc Spoleczna': 10,
      'Administracja publiczna': -10,
      'Edukacyjna opieka wychowawcza': 1,
      'Obsługa długu publicznego': -10,
      'Pozostałe zadania w zakresie polityki społecznej': 10,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 10,
      'Pozostale wydatki': 0
    })
  },
  '70-79': {
    size: 43142,
    voters: 0.3,
      weights: asMap({
      'Oświata i wychowanie': 1,
      'Transport i łączność': 10,
      'Gospodarka komunalna i ochrona środowiska': 2,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 2,
      'Gospodarka mieszkaniowa': 7,
      'Pomoc Spoleczna': 10,
      'Administracja publiczna': -10,
      'Edukacyjna opieka wychowawcza': 5,
      'Obsługa długu publicznego': -10,
      'Pozostałe zadania w zakresie polityki społecznej': 10,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 8,
      'Pozostale wydatki': 0
    })
  },
  '>80': {
    size: 32689,
    voters: 0.3,
      weights: asMap({
      'Oświata i wychowanie': 0,
      'Transport i łączność': 10,
      'Gospodarka komunalna i ochrona środowiska': 5,
      'Różne rozliczenia': 0,
      'Kultura i ochrona dziedzictwa narodowego': 2,
      'Gospodarka mieszkaniowa': 8,
      'Pomoc Spoleczna': 10,
      'Administracja publiczna': -10,
      'Edukacyjna opieka wychowawcza': 0,
      'Obsługa długu publicznego': -10,
      'Pozostałe zadania w zakresie polityki społecznej': 10,
      'Bezpieczeństwo publiczne i ochrona przeciwpożarowa': 8,
      'Pozostale wydatki': 0
    })
  }
}